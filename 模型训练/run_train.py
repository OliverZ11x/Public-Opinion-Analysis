...
from paddlenlp.prompt import template
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_args.model_name_or_path, truncation=True)
    ...
    #         truncation=True,  # 自动截断超过最大长度的输入
    #     )
    #     # 如果需要 choices 字段，确保在这里添加
    #     if "choices" in examples:
    #         inputs["choices"] = examples["choices"]
    #     # 如果需要 choices 字段，确保在这里添加
